# 🤖 서브에이전트 시스템 (Sub-Agent System)

## 자동 실행 규칙
모든 작업 시작 전에 `.agent/skills/SKILL.md`를 참조하여 다음을 자동으로 수행합니다:
1. **설계도 확인**: `README.md` 로드
2. **규칙 확인**: 이 `GEMINI.md` 파일 로드
3. **API 문서 확인**: 관련 KIS API 문서 로드 (해당 시)

## 사용 가능한 워크플로우 (슬래시 명령어)
| 명령어 | 설명 |
|--------|------|
| `/feature-dev` | 새 기능 개발 워크플로우 (계획→구현→테스트→문서화) |
| `/sync-rules` | 규칙/설계도 강제 동기화 |
| `/code-review` | 코드 품질 검증 워크플로우 |
| `/deploy` | Vercel 프로덕션 배포 워크플로우 |
| `/docs` | JSDoc 및 코드 문서화 워크플로우 |
| `/performance` | 번들 크기 및 렌더링 성능 분석 워크플로우 |
| `/security` | 보안 취약점 스캔 및 검증 워크플로우 |

## 서브에이전트 역할
- **Planner**: 요구사항 분석, 구현 계획 수립
- **Developer**: 코드 작성, 기능 구현
- **Reviewer**: 코드 리뷰, 규칙 준수 검증
- **Browser**: UI 테스트, 스크린샷 캡처
...
# 한국투자증권 API 문서
kis폴더쪽에 API 의 상세 정보가 적혀 있는 마크다운문서가 있습니다.
관련 내용 구현하기전에는 항상 해당 API 문서를 확인하면서 개발해주세요.

# 설계도 업데이트
작업 완료후에는 설계도(README.md)에 업데이트 할 내용이 있다면 업데이트 해주세요.
...
## 2. Git 커밋 컨벤션 (Commit Convention)
우리는 **Udacity 스타일 가이드**와 **Gitmoji**를 결합하여 사용하며, 나중에 들어올 협업자를 위해 일관된 메시지 구조를 유지합니다.

### 커밋 메시지 구조
```text
<Emoji> <Type>: <Subject> (빈 줄)
<body> (빈 줄)
<footer>
```
...
### 작성 규칙
- **언어**: 태그(Type)를 제외한 모든 내용은 **한국어**로 작성합니다. 영문 표기보다 명확한 의도 전달을 우선합니다.
- **Subject**: 최대 50자 내외, 마침표 및 특수기호 제거. '~함', '~함'과 같은 개조식 구문으로 간결하게 작성합니다.
- **Body**: 한 줄 당 72자 내외. '어떻게'보다 '무엇을', '왜' 변경했는지를 한국어로 최대한 상세히 설명합니다.
- **Footer**: Optional. 이슈 트래커 ID 작성 (예: Resolves: #123, Related to: #45).
...
## 1. 자동 브라우저 테스트 제한
- **사용자의 명시적인 요청이 없는 한 브라우저 자동 테스트(browser_subagent)를 수행하지 마세요.**
- 토큰 절약을 위해, 개발 서버 실행(`npm run dev`) 후 "서버가 정상적으로 시작되었습니다"라고만 보고하고, 실제 확인은 사용자에게 맡기세요.
- UI 확인이 꼭 필요한 경우에만 사용자에게 허락을 구한 뒤 수행하세요.
