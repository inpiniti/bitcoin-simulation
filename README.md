...
- **AI 시뮬레이션**: 학습된 모델을 시뮬레이터에 적용하여 백테스팅 수행 (확률 기반 매매).
...
#### Sidebar (전략 설정)
*   **매매 전략 모드 선택**:
    *   **일반 전략 (Rule-based)**: BB, Trend, RSI 등의 고전적 기술적 지표 필터 사용.
    *   **AI 딥러닝 전략 (Deep Learning)**: 학습된 모델(XGBoost)을 선택하고 확률 임계값(Threshold)을 설정하여 시뮬레이션.
...
*   **동작 방식**:
    ...
    4.  **UI 업데이트**:
        ...
        *   **스파크라인 미니 차트**: 각 종목의 가격 추이를 테이블 내 작은 그래프로 표시. 순수 SVG 기반으로 성능 최적화 (300개 캔들 → 30개 포인트 샘플링).
