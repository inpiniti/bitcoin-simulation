...
2.  **간격별 데이터 생성 (Aggregation)**
    ...
    *   **자동 생성 (Auto Generation)**:
        *   기본 데이터(Coin: 1m, Stock: 1d) 로드가 완료되면, **모든 파생 간격 표본(5m, 1h, 1w 등)을 자동으로 생성**하여 즉시 시뮬레이션 가능한 상태로 만듦.
    *   **자동 선택 (Auto Selection)**:
        *   데이터 준비가 완료되면 **기본 간격(Coin: 1m, Stock: 1d)을 자동으로 선택(Active)**하여 즉시 시뮬레이션 및 데이터 확인이 가능하도록 함.
...
*   **동작 방식**:
    ...
    3.  **분석 (Parsing)**:
        *   각 종목 데이터에 대해 `median`, `slope`, `bb` 등 지표 계산.
        *   **현재 Sidebar에서 선택된 전략** 및 **ActivityBar에서 선택된 간격**을 기준으로 **오늘(Last Candle)**의 매매 신호를 판별.
    4.  **결과 표시**:
        *   `EditorArea`에 분석 결과 테이블 출력.
        *   **컬럼**: `종목(Ticker)` | `신호(Signal)` | `현재가(Price)` | `기울기(Slope)` | `BB 위치`
