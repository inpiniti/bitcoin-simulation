...
#### 설정 (Settings)
*   **매매 전략**: 현재 사이드바에 설정된 전략 옵션(BB, RSI, 추세 등)을 사용.
...
3.  **시장 분석 (Step 4 & 5)**:
    *   설정된 티커 그룹의 1년치 데이터 로드 (API 비용 고려).
    *   전략(Strategy)에 따라 전체 종목에 대해 `BUY` / `SELL` 신호 산출.
    *   **BUY 필터**: 신호가 `BUY`이면서, 현재 보유하지 않은 종목.
4.  **매수 실행 (Step 6)**:
    *   `BUY` 대상 종목의 실시간 현재가 조회.
    *   설정된 금액/수량에 맞춰 주문 수량 계산.
    *   KIS 매수 주문 전송.
5.  **매도 실행 (Step 7 & 8)**:
    *   **SELL 필터**: 신호가 `SELL`이면서, 현재 보유 중인 종목.
    *   `SELL` 대상 종목의 실시간 현재가 조회.
    *   보유 수량 전량 매도 주문 전송.
6.  **결과 로그**:
