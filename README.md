...
*   Activity Bar에서 선택한 모드에 따라 다른 뷰 렌더링:
    ...
    *   **실시간 시뮬레이션 차트 모드**: 1분봉 자동 갱신 + AI 예측 차트
...
*   **워크플로우**:
    1.  1분마다 1분봉 데이터 자동 조회 (일시정지 가능)
    2.  차트에 있는 'AI 예측 로드' 버튼 클릭 시 분 단위 예측 데이터 조회
    3.  모든 데이터 로드 완료 후 차트 렌더링
*   **기능**:
    *   ⏸️ **일시정지/재개**: 자동 새로고침 제어
    *   🔄 **수동 새로고침**: 즉시 데이터 갱신
    *   📊 **AI 예측 로드**: TimesFM 모델 기반 분 단위 예측
    *   ⏱️ **카운트다운 타이머**: 다음 업데이트까지 남은 시간 표시
*   **표시 정보**: 현재가, 일중 변동률, 데이터 개수, 마지막 업데이트 시간
