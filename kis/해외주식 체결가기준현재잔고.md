...
| **모의 Domain** | https://openapivts.koreainvestment.com:29443<br>(output3만 이용 가능) |
| **실전 TR ID** | CTRP6504R |
| **모의 TR ID** | VTRP6504R |
| **Format** | JSON |
...
⚠️ **(※모의계좌의 경우 output3(외화평가총액 등 확인 가능)만 정상 출력됩니다. 잔고 확인을 원하실 경우에는 해외주식 잔고[v1_해외주식-006] API 사용을 부탁드립니다.)**
...
5. 해외주식 담보대출 매도상환 체결내역은 해당 잔고화면에 반영되지 않습니다. 결제가 완료된 이후 외화잔고에 포함되어 반영되오니 참고하여 주시기 바랍니다.
...
7. 미국은 메인 시스템이 아닌 별도 시스템을 통해 거래되므로, 18시 10~15분 이후 발생하는 미국 매매내역은 해당 화면에 실시간으로 반영되지 않으니 하단 내용을 참고하여 안내하여 주시기 바랍니다.
   ...
   **[외화잔고 및 해외 유가증권 현황 조회]**
   - 일반/통합증거금 계좌 : 미국장 종료 + 30분 후 부터 조회 가능
     단, 통합증거금 계좌에 한해 주문금액은 외화잔고 항목에 실시간 반영되며, 해외 유가증권 현황은 반영되지 않아 해외 유가증권 평가금액이 과다 또는 과소 평가될 수 있습니다.
   - 애프터연장 신청계좌 : 실시간 반영
     단, 시스템정산작업시간(23:40~00:10) 및 거래량이 많은 경우 메인시스템에 반영되는 시간으로 인해 차이가 발생할 수 있습니다.
...
| **TR_MKET_CD** | 거래시장코드 | String | Y | 2 | **[NATN_CD 000 설정]**<br>00 : 전체<br><br>**[NATN_CD 840(미국) 설정]**<br>00 : 전체<br>01 : 나스닥(NASD)<br>02 : 뉴욕거래소(NYSE)<br>03 : 미국(PINK SHEETS)<br>04 : 미국(OTCBB)<br>05 : 아멕스(AMEX)<br><br>**[NATN_CD 156(중국) 설정]**<br>00 : 전체<br>01 : 상해B<br>02 : 심천B<br>03 : 상해A<br>04 : 심천A<br><br>**[NATN_CD 392(일본) 설정]**<br>01 : 일본<br><br>**[NATN_CD 704(베트남) 설정]**<br>01 : 하노이거래<br>02 : 호치민거래소<br><br>**[NATN_CD 344(홍콩) 설정]**<br>01 : 홍콩<br>02 : 홍콩CNY<br>03 : 홍콩USD |
| **INQR_DVSN_CD** | 조회구분코드 | String | Y | 2 | 00 : 전체<br>01 : 일반해외주식<br>02 : 미니스탁 |

#### Request URL 예시

```
GET /uapi/overseas-stock/v1/trading/inquire-present-balance?CANO=12345678&ACNT_PRDT_CD=01&WCRC_FRCR_DVSN_CD=02&NATN_CD=840&TR_MKET_CD=00&INQR_DVSN_CD=00
```
...
| **output2** | 응답상세2 | Array | Y | - | 통화별 정보 |
| **output3** | 응답상세3 | Object | Y | - | 전체 평가 정보 |

### output1 배열 항목 (체결기준 잔고)
...
| **evlu_pfls_amt2** | 평가손익금액2 | String | Y | 31 | 해당 종목의 매입금액과 평가금액의 외화기준 비교 손익 |
...
| **item_lnkg_excg_cd** | 종목연동거래소코드 | String | Y | 4 | prdt_dvsn(상품구분) : 직원용 데이터 |

### output2 배열 항목 (통화별 정보)
  ...
  "output1": [
    {
      ...
      "frcr_evlu_amt2": "1550.00",
      "evlu_pfls_amt2": "50.00",
      "evlu_pfls_rt1": "3.33",
      "pdno": "AAPL",
      "bass_exrt": "1300.50",
      "buy_crcy_cd": "USD",
      "ovrs_now_pric1": "155.00",
      "avg_unpr3": "150.00",
      "tr_mket_name": "나스닥",
      "natn_kor_name": "미국",
      "ovrs_excg_cd": "NASD"
  ...
  "output2": [
    {
      "crcy_cd": "USD",
      "crcy_cd_name": "미국달러",
      "frcr_buy_amt_smtl": "1500.00",
      "frcr_sll_amt_smtl": "0.00",
      "frcr_dncl_amt_2": "5000.00",
      "frst_bltn_exrt": "1300.50"
    }
  ],
  "output3": {
    "pchs_amt_smtl": "1950750",
    "evlu_amt_smtl": "2015775",
    "evlu_pfls_amt_smtl": "65025",
    "tot_asst_amt": "8515775",
    "evlu_erng_rt1": "3.33",
    "frcr_evlu_tota": "6550.00"
  }
}
```
...
if result["rt_cd"] == "0":
    ...
    for stock in result["output1"]:
        print(f"종목: {stock['prdt_name']}")
        print(f"보유수량: {stock['ccld_qty_smtl1']}")
        print(f"평가손익: {stock['evlu_pfls_amt2']} ({stock['evlu_pfls_rt1']}%)")
        print("---")
    
    print("\n=== 전체 평가 ===")
    output3 = result["output3"]
    print(f"총평가금액: {output3['evlu_amt_smtl']}원")
    print(f"총평가손익: {output3['evlu_pfls_amt_smtl']}원")
    print(f"수익률: {output3['evlu_erng_rt1']}%")
else:
...
```bash
curl -X GET "https://openapi.koreainvestment.com:9443/uapi/overseas-stock/v1/trading/inquire-present-balance?CANO=12345678&ACNT_PRDT_CD=01&WCRC_FRCR_DVSN_CD=02&NATN_CD=840&TR_MKET_CD=00&INQR_DVSN_CD=00" \
...
Response Header의 `tr_cont` 값이 `F` 또는 `M`이면 다음