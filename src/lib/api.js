 ...
 * 주식 과거 데이터 조회 (일봉)
 * 
 * @param {string} ticker - 종목 코드 (예: "AAPL", "BTC-USD")
 * @param {number|string} days - 조회 기간. 숫자일 경우 n일, 'max'일 경우 전체 기간(약 55년)을 조회합니다.
 * @returns {Promise<Array>} 정규화된 캔들 데이터 배열
 * 
 * @description
 * days가 'max'인 경우 Yahoo Finance API가 월봉으로 해상도를 낮추는 것을 방지하기 위해 
 * 명시적으로 20000d(약 55년)를 사용합니다.
 */
export async function fetchStockHistory(ticker, days = 365) {
    // 'max' 요청 시 Yahoo Finance가 데이터 해상도를 마음대로 조정(예: 월봉)하는 것을 방지하기 위해
    // 명시적으로 충분히 큰 일수(약 55년)를 지정합니다.
    const range = days === 'max' ? '20000d' : (typeof days === 'number' ? `${days}d` : days);
